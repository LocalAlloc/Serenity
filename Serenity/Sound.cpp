...
HANDLE g_terminateEvent = NULL; // Global event handle

HANDLE Voice = NULL; // Thread handle
...
//VOID WINAPI AudioPayloadThread(VOID)
DWORD WINAPI AudioPayloadThread(LPVOID lpParam)
{
	g_terminateEvent = CreateEvent(NULL, TRUE, FALSE, NULL);
	for (;;)
	{
		DWORD result = WaitForSingleObject(g_terminateEvent, 0);
		if (result == WAIT_OBJECT_0) {
			//break; // Exit the loop when the event is signaled
			return 0;
		}
